...
function cf_login() {
    ...
    if [ ! -z "$ISOLATION_SEGMENT" ]; then
        ...
        local space_domain=${domains[0]^^}
        CF_ORG="${COMPLIANCE}-${space_domain}${ISOLATION_SEGMENT}"
        echo "NEW CF_ORG = $CF_ORG"
    fi
...
function create_autoscaler_service() {
  ...
  autoscaler_service_count=$(cf services | awk '/app-autoscaler/ {print $1}' | wc -l || true)
  ...
  if [[ autoscaler_service_count -eq 0 ]]; then
    ...
    # If the service exists, make sure that the name is what is expected. Someone could have created this already with a different name and service binding will fail.
    local service_name
    service_name=$(cf services | awk '/app-autoscaler/ {print $1}' || true)
    if [[ $service_name != ${1}-autoscaler ]]; then
      cf rename-service "${service_name}" "${1}-autoscaler"
    fi
  fi
...
function bind_autoscaler_service() {
  echo "##teamcity[progressStart 'bind_autoscaler_service $1-autoscaler']"
...
function install_autoscaler_plugin() {
  echo "##teamcity[progressStart 'install autoscaler cf plugin']"
...
function main() {
    ...
    do
        ...
        cf_login "https://${CF_API_PREFIX}.${cf_domain}" "${cf_domain}"
